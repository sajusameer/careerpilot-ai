...
function LoginForm() {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      // 1. Attempt to sign in directly (fast-path if user exists)
      ...
      if (signInRes.error) {
        // 2. If login failed, register/auto-provision the demo account
        const registerRes = await authClient.signUp.email({
          email: demoEmail,
          password: demoPassword,
          name: demoName,
        });

        // Continue to sign-in even if registration says 'user already exists'
        if (registerRes.error && !registerRes.error.message?.toLowerCase().includes("already")) {
          throw new Error(registerRes.error.message || "Failed to create demo credentials.");
        }

        // 3. Retry signing in after registration completes
        ...
        if (retryRes.error) {
          throw new Error(retryRes.error.message || "Failed to log in after demo registration.");
        }
      }
    } catch (err: any) {
      setSubmitError(err.message || "Failed to authenticate demo session. Please try again.");
      setIsSubmitting(false);
    }
  };
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 px-6 py-12 text-slate-100 selection:bg-indigo-500/30 selection:text-indigo-200 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 p-8 rounded-2xl shadow-2xl relative z-10">
        ...
        {redirect === "/dashboard/resume-analyzer" && (
          <div className="mb-6 p-4 rounded-xl border border-indigo-500/30 bg-indigo-500/5 text-xs text-indigo-300 flex items-start gap-2.5 animate-fadeIn">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4.5 w-4.5 text-indigo-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>Please log in to use the AI Resume Analyzer.</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <a href="#" className="text-xs font-medium text-cyan-400 hover:text-cyan-300 hover:underline transition-colors">
                Forgot password?
              </a>
            </div>
            <div className="relative">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-lg bg-indigo-650 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold text-xs shadow-lg shadow-indigo-650/15 hover:shadow-indigo-650/25 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 flex justify-center items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {isSubmitting ? (
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full py-2.5 px-4 mt-2.5 rounded-lg border border-cyan-500/30 bg-cyan-500/5 hover:bg-cyan-500/15 text-cyan-400 font-semibold text-xs transition-all duration-200 flex justify-center items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            Demo Login (Try Copilot Instantly)
          </button>
        </form>
        ...
        <div className="relative my-6 text-center">
          ...
          <span className="relative bg-slate-900 px-3 text-[10px] text-slate-500 uppercase tracking-widest">
            Or continue with
          </span>
        </div>
        ...
        <button
          ...
          className="w-full py-2.5 px-4 rounded-lg border border-slate-800 bg-slate-950 text-slate-350 hover:text-white hover:bg-slate-900 font-semibold text-xs transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer"
        >
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-955 text-slate-100">
        <span className="flex h-12 w-12 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mb-4" />
        <p className="text-sm text-slate-400 font-medium">Loading secure sign-in portal...</p>
      </div>
    }>
      <LoginForm />
    </Suspense>
